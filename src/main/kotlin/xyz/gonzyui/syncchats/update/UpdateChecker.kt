...
class UpdateChecker {
    ...
    fun checkForUpdates() {
        ...
        try {
            ...
            if (currentVersion != latestVersion) {
                val message = MessageManager.getSystemMessage("update_available", mapOf("version" to latestVersion))
            ...
            val message = MessageManager.getSystemMessage("update_error")
            logToMinecraft(message)
            e.printStackTrace()
        }
    }
    ...
    private fun createDiscordMessage(message: String): JSONObject {
        return JSONObject().apply {
            put("username", ConfigManager.getConfig().getString("discord.bot_name") ?: "SyncChats")
            put("avatar_url", ConfigManager.getConfig().getString("discord.bot_avatar") ?: "https://www.spigotmc.org/data/resource_icons/123/123042.jpg?1741111607")
            put("content", message)
        }
    }
